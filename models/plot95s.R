...
ci95s= read.csv('basePredictions_95ci.csv',header=F)
...
dodge = position_dodge(0.61)
### base model
ggplot(ci95s, aes(x=measurement, y=mean,fill=measurement,alpha=cond.model))+
  geom_bar(position=dodge,stat='identity',width=0.6)+
  geom_bar(position=dodge,stat='identity',width=0.6,color=I("black"), show_guide=F)+  
  geom_errorbar(aes(ymin=lbound,ymax=ubound),width=0.2,
                position=dodge)+
  scale_alpha_manual(values=c(1,0.3),name='')+
